...
export async function generatePlanAction(
  ...
  try {
    ...
    console.log("[1] Starting generation flow... Calling Gemini...");
    ...
    console.log("[2] API responded successfully! Gemini plan generated.");
      ...
      generated.topPlacesToVisit = await Promise.all(
        generated.topPlacesToVisit.map(async (place) => {
          try {
            ...
            console.error("CRITICAL FETCH ERROR:", mapError?.message, mapError?.stack);
          }
    ...
    try {
      ...
      console.log("[3] UI state updated. Plan saved successfully.");
