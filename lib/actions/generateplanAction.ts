...
export async function generatePlanAction(
  ...
  const signal: { timerId?: NodeJS.Timeout } = {};

  try {
    console.log("[1] Starting Gemini generation flow...");
    const prompt = [
    ...
    try {
      ...
      console.log("[3] UI state / Database updated.");
    ...
    if (signal.timerId) clearTimeout(signal.timerId);
  }
}
