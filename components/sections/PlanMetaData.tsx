...
import { updateTripInFirestore } from "@/lib/firebase/firestore-db";
...
const PlanMetaData = ({
  ...
  const onChangeTravelDates = async (e: DateRange | undefined) => {
    ...
    setSelectedDates(e);
    if (e.from && e.to) {
      await updateTripInFirestore(planId, {
...
const PublishPlan = ({
  ...
  const onClickChangePrivacy = async () => {
    try {
      ...
      console.error(error);
      toast({
        title: "Unable to update plan privacy",
        description: "Please try again after the plan is saved.",
      });
    } finally {
      setOpen(false);
    }
...
const Preferences = ({
  ...
  const saveActivityPreferences = async () => {
    ...
    await updateTripInFirestore(planId, {
      activityPreferences: selectedActivityPreferencesIds,
    });
    toast({
