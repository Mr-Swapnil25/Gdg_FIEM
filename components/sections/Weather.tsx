...
const Weather = ({placeName}: {placeName: string | undefined}) => {
  ...
  useEffect(() => {
    ...
    console.log("[WEATHER] Starting weather fetch for:", placeName);
    
    const apiKey = process.env.NEXT_PUBLIC_OPENWEATHER_API_KEY;
    if (!apiKey) {
      console.warn("[WEATHER] No API key provided, skipping weather fetch");
      setPlanState((state) => ({...state, weather: true}));
      return;
    }
    
    // Step 4: Add a Timeout Failsafe (Fail-Fast) - 30-second timeout
    ...
    fetchWithTimeout
      .then((response) => {
        console.log("[WEATHER] Response received, parsing JSON...");
        return (response as Response).json();
      })
      .then((data) => {
        console.log("[WEATHER] Weather data parsed successfully");
        if (data) {
          setWeatherData(data);
        }
      })
      .catch((e) => {
        console.error("[WEATHER] CRITICAL FETCH ERROR:", e instanceof Error ? e.message : e, e instanceof Error ? e.stack : "");
        setWeatherData(undefined);
      })
      .finally(() => {
        console.log("[WEATHER] Finally block executed - setting weather state to complete");
        setPlanState((state) => ({...state, weather: true}));
      });
  }, [placeName]);
