...
const NewPlanForm = ({closeModal}: {closeModal: Dispatch<SetStateAction<boolean>>}) => {
  ...
  async function onSubmitEmptyPlan(values: formSchemaType) {
    ...
    try {
      ...
      const fetchPromise = generateEmptyPlanAction(values, userId);
      const timeoutPromise = new Promise<never>((_, reject) => {
        timerId = setTimeout(() => reject(new Error("Request Timed Out")), 30000);
      });

      const planId = await Promise.race([fetchPromise, timeoutPromise]);
  ...
  async function onSubmitAIPlan(values: formSchemaType) {
    ...
    try {
      ...
      const fetchPromise = generatePlanAction(values, userId);
      const timeoutPromise = new Promise<never>((_, reject) => {
        timerId = setTimeout(() => reject(new Error("Request Timed Out")), 30000);
      });
