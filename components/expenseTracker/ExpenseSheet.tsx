...
import {addExpenseToTrip, updateExpenseInTrip} from "@/lib/firebase/firestore-db";
...
export default function ExpenseSheet({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    if (edit) {
      await updateExpenseInTrip(data!._id, {
        planId,
    ...
    await addExpenseToTrip(planId, {
      ...
      date: values.date.toISOString(),
      whoSpent:
        values.userId === user.uid ? user.displayName ?? user.email ?? "You" : values.userId,
    });
  };
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent>
        <SheetHeader>
          ...
          <SheetDescription>
            {edit ? "Edit" : "Add"} your trip expense in INR.
          </SheetDescription>
        </SheetHeader>
        <Form {...form}>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        {user && <UserDropdown userId={user.uid} planId={planId} />}
                      </SelectContent>
                    </Select>
                  </FormControl>
                  <FormMessage />
