...
import {fetchPreferredCurrency, fetchTripExpenses} from "@/lib/firebase/firestore-db";
import {ExpenseDoc} from "@/lib/types/firestore";
...
const ExpenseSection = ({ planId }: { planId: string }) => {
  const [data, setData] = useState<(ExpenseDoc & {whoSpent: string})[] | undefined>();
  const [preferredCurrency, setPreferredCurrency] = useState("INR");

  useEffect(() => {
    fetchTripExpenses(planId).then(setData).catch(() => setData([]));
    fetchPreferredCurrency(planId).then(setPreferredCurrency).catch(() => setPreferredCurrency("INR"));
  }, [planId]);
