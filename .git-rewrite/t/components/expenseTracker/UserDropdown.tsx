"use client";

import { SelectItem } from "@/components/ui/select";
import { UserIcon } from "lucide-react";
import {useEffect, useState} from "react";
import {fetchCurrentUserProfile, fetchTripUsers} from "@/lib/firebase/firestore-db";
import {Doc} from "@/lib/types/firestore";

const UserDropdown = ({
  planId,
  userId,
}: {
  planId: string;
  userId: string;
}) => {
  const [sharedUserList, setSharedUserList] = useState<Doc<"users">[]>([]);

  useEffect(() => {
    fetchCurrentUserProfile(userId)
      .then((currentUser) => fetchTripUsers(planId, currentUser))
      .then(setSharedUserList)
      .catch(() => setSharedUserList([]));
  }, [planId, userId]);

  const getDisplayName = (userObject: Doc<"users">) => {
    if (!userObject.firstName && !userObject.lastName) return userObject.email;
    if (userObject.firstName && userObject.firstName.length > 0)
      return (
        userObject.firstName +
        (userObject.lastName ? ` ${userObject.lastName}` : "")
      );
  };

  return sharedUserList?.map((userObject) => (
    <SelectItem value={userObject.userId} key={userObject.userId}>
      <div className="flex gap-2 items-center">
        <UserIcon className="h-4 w-4" />
        <span>
          {getDisplayName(userObject)} {userObject.IsCurrentUser && "(You)"}
        </span>
      </div>
    </SelectItem>
  ));
};

export default UserDropdown;
