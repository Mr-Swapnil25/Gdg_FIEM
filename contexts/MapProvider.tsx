  ...
  if (googleMapsApi.isKeyMissing || googleMapsApi.loadError || googleMapsApi.isTimedOut) {
    return (
      <div className="flex h-full items-center justify-center rounded-md border border-dashed p-4 text-center text-sm text-muted-foreground">
        {googleMapsApi.loadError ? `Error loading maps: ${googleMapsApi.loadError.message}` : "Google Maps is unavailable. Enter destination manually."}
      </div>
    );
  }

  if (!googleMapsApi.isLoaded) {
    return <div className="flex h-full items-center justify-center rounded-md border border-dashed p-4 text-center text-sm text-muted-foreground">Loading Maps...</div>;
  }
